...
public class Exporter {
	...
	private static ArrayList<ConnectionHolder> synapses = new ArrayList<ConnectionHolder>();
	...
	private static HashT<Cell, Integer> inhibID = new HashT<Cell, Integer>();
	private static HashT<Cell, Integer> excitID = new HashT<Cell, Integer>();
	...
	public static void saveExport() {
		...
		try {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				OutD.println("Could not export NeuroMl description");
				return;
			}
	 ...
	 * @return
	 */
	public static ArrayList<ConnectionHolder> dispatchSynapsesOfThisNeuriteElement(ArrayList<ConnectionHolder> chs, NeuriteElement ne){
		// all excresc.
//		ArrayList<Excrescence> exs = new ArrayList<Excrescence>();
//		ini.cx3d.biology.synapse2.Excrescence t = ne.getPhysicalCylinder().getExcrescence();
//		exs.add(t);
...
//				Cell postCell = ex.getEx().getPo().getCellularElement().getCell();
...
//					}
//				}else{
//					OutD.println("Exporter.dispatchSynapsesOfThisNeuriteElement()");
//					if(postCell.getNeuroMLType() == Cell.ExcitatoryCell){
//						// In -> Ex
...
//		}
//		return chs;	
		return new ArrayList<ConnectionHolder>();
	}
	...
	public static StringBuilder neuroML_level3_export(){
		...
		int excitCounter = 0;
		int inhibCounter = 0;
		for (Long i: ecm.cellList.keySet()) {
			Cell c = ecm.cellList.get(i);
		...
		for (Long i: ecm.cellList.keySet()) {
			...
			ArrayList<NeuriteElement> nes = c.getNeuriteElements();
			ArrayList<ConnectionHolder> chs = new ArrayList<ConnectionHolder>(); // not really used, necessary for next method
