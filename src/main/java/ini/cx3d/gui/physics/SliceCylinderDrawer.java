...
public class SliceCylinderDrawer extends Drawer{
	...
	public SliceCylinderDrawer()
	{
		super();
		cube = genCube();
		this.name = "Slice Cylinders";
	}
	@Override
...
//								drawCuboid(s.getSlotCuboid(i));
								if(s.doesSlotContain(i))
								{
									...
									for(int f=0;f<temp.size();f++)
									{
										try
										{
											...
											if(node instanceof PhysicalCylinder)
											{
												processView((PhysicalCylinder)node,g);
											}
										}catch (Exception e) {
	...
	public void processView(PhysicalCylinder s,Graphics g)
	{
